...
public class tictactoe {
   ...
   public static void playerMove(int currentTurn) {
      ...
      do {
         ...
         if (row >= 0 && row < ROWS && col >= 0 && col < COLS && board[row][col] == EMPTY) {
            currntRow = row;
            currentCol = col;
            board[currntRow][currentCol] = currentTurn;
            validInput = true;
         } else {
   ...
   public static void updateGame(int currentTurn, int currentRow, int currentCol) {
      if (hasWon(currentTurn, currentRow, currentCol)) {  // check if winning move
         currentState = (currentTurn == CROSS) ? CROSS_WON : CIRCLE_WON;
      } else if (isDraw()) {
         currentState = DRAW;
      }
   }
   ...
   public static boolean hasWon(int currentTurn, int currentRow, int currentCol) {
      return (board[currentRow][0] == currentTurn
                   && board[currentRow][1] == currentTurn
                   && board[currentRow][2] == currentTurn
              || board[0][currentCol] == currentTurn
